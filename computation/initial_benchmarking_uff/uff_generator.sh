...
ELIST=$(awk '
  function norm(s){
    ...
    gsub(/[^A-Za-z]/,"",s); s=tolower(s);
    return (s=="si"?"Si":s=="c"?"C":s=="o"?"O":s=="h"?"H":"");
  }
  $1=="Masses" { inblk=1; started=0; next }     # enter Masses section
  inblk && NF==0 { if (started) inblk=0; next } # blank before entries: skip; blank after: end
  inblk && $1 ~ /^[0-9]+$/ {
    started=1;
    split($0,p,"#"); sym=(length(p)>=2?norm(p[2]):"");
    if (sym==""){ printf("ERROR: Unknown/blank element for type %s\n",$1) > "/dev/stderr"; exit 1 }
    a[$1]=sym; if ($1>max) max=$1; next
  }
  END{
    for (i=1;i<=max;i++){
      if (!(i in a)){ printf("ERROR: Missing mass line for type %d\n",i) > "/dev/stderr"; exit 1 }
      printf("%s%s",(i>1?" ":""),a[i]);
    }
  }
' "$DATAFILE") || exit 1
...
dump        atom_pos all extxyz 100 ./output/FILENAME.lammpstrj
dump_modify atom_pos element ELIST mass yes
thermo          1000
...
sed -i "s/ELIST/${ELIST}/g" in.${filename}
