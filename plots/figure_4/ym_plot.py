...
data = [
    ...
    ('no_relax', 'Simple Relaxation'),
    ('unmelt_equil', 'NVT and NPT @ 300K')
]
DOT_SIZE = 5
labels = {
    "EXP" : "07-26-18-00-30",
...
for filename,label in data:
    ...
    plt.text(0.825,58,f'RMSE (polynomial): {rmse_poly:.3f}\nRMSE (exponential): {rmse_exp:.3f}',fontsize=10)

    plt.legend(prop={'size':11})
    plt.title(f'Density vs Young Modulus ({label})',size=title_size)
    plt.xlabel('Density / g cm$^{-3}$',size=axis_size)
    plt.ylabel("Young Modulus / GPa",size=axis_size)
    plt.xlim((0.8,2.4))
    plt.ylim((0,100))
    plt.savefig(f'./plots/eval_{filename}.png')
    plt.clf()
