...
filenames = [
    ("Full Routine","initial_with_full_routine"),
    ("Minimization","initial_with_minimize"),
    ("NVT and NPT @ 300K","initial_with_npt"),
]
...
panel_letters = ['(a)','(b)','(c)']
for i, (title,filename) in enumerate(filenames):
    ...
    plt.title(f'Density vs Young Modulus ({title})',size=title_size)
    plt.xlabel('Density / g cm$^{-3}$',size=axis_size)
    plt.ylabel("Young Modulus / GPa",size=axis_size)
    plt.text(0.8125,88,f'RMSE (polynomial): {rmse_poly:.3f}\nRMSE (exponential): {rmse_exp:.3f}',fontsize=10)
    plt.xlim((0.8,2.4))
    plt.ylim((0,150))
    plt.legend()
    plt.savefig(f'./plots/qd_{filename}.png')
    plt.clf()