...
ml_x,ml_y = [],[]
avg_ym = []
with open(f'./data/ml.txt','r') as f:
    # plt.scatter(exp_density,exp_ym,label='Experiments',s=20)
    defects = {}
    for line in f.readlines():
        tokens = line.split(',')
        defect = labels[tokens[2].strip()]
        if defect not in defects:
            defects[defect] = [[],[]]
        defects[defect][0].append(float(tokens[0]))
        defects[defect][1].append(float(tokens[1]))
...
y_poly_pred = poly_f(np.array(ml_x),*poly_param)
y_exp_pred = exp_f(np.array(ml_x),*exp_param)
rmse_poly = np.sqrt(mean_squared_error(ml_y,y_poly_pred))
rmse_exp = np.sqrt(mean_squared_error(ml_y,y_exp_pred))
...
plt.title(f'Density vs Young Modulus (MD Simulation (M3GNet))',size=title_size)
plt.xlabel('Density / g cm$^{-3}$',size=axis_size)
plt.ylabel("Young Modulus / GPa",size=axis_size)
plt.xlim(x_axis_range)
plt.ylim(y_axis_range)
plt.savefig(f'./plots/qd_ml.png')
plt.clf()
...
plt.savefig('./plots/avg_qd_ym.png')

print(poly_label,exp_label,sep='\n')